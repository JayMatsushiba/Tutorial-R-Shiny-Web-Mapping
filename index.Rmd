...
In typical R code, we can run our code line by line. This is called `interactive` mode. That means we can write one line of code and run it.
```{r}
x <- "hello world"
```
...
However, when we create Shiny apps and then run them, we don't have the option of adding new lines of code and building upon the previous steps while the app is running. In other words, the Shiny app will need to contain all of the code to load in files, do the intended operations on them, etc. This will become apparent as we go through the lab. 
